...
void Surface::intersectBVH(uint32_t nodeIdx, Ray& ray, Interaction& si,int option)
{
    BVHNode& node = this->nodes[nodeIdx];
    ...
    if (node.primCount != 0) {
        ...
        for (uint32_t i = 0; i < node.primCount; i++) {
            Vector3f vt1,vt2,vt3;
            Vector2f uv1,uv2,uv3;
            vt1 =  this->tris[this->getIdx(i + node.firstPrim)].v1;
            vt2 =  this->tris[this->getIdx(i + node.firstPrim)].v2;
            vt3 =  this->tris[this->getIdx(i + node.firstPrim)].v3;
            ...
            if (siIntermediate.t <= ray.t && siIntermediate.didIntersect) {
                ...
                if(!this->hasDiffuseTexture()){
                    si.texture = this->diffuse;
                }
                else{
                    float ar1,ar2,ar3,artotal;
                    ar1 = Cross((si.p - vt2), (vt3 - vt2)).Length() / 2;
                    ar2 = Cross((si.p - vt1), (vt3 - vt1)).Length() / 2;
                    ar3 = Cross((si.p - vt1), (vt2 - vt1)).Length() / 2;
                    artotal = Cross((vt3 - vt1), (vt2 - vt1)).Length() / 2;

                    si.uv = (ar1*uv1 + ar2*uv2 + ar3*uv3)/artotal;
                    if(option == 0){
                        si.texture = this->diffuseTexture.nearestNeighbourFetch(si.uv);
                    }
                    else if(option == 1){
                        si.texture = this->diffuseTexture.bilinearFetch(si.uv);
                    }
                }
                ray.t = si.t;
            }
        ...
        this->intersectBVH(node.left, ray, si, option);
        this->intersectBVH(node.right, ray, si, option);
    }
}

Interaction Surface::rayIntersect(Ray& ray,int option)
{
    Interaction si;
    si.didIntersect = false;
    this->intersectBVH(0, ray, si, option);
