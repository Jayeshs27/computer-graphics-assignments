...
void Scene::parse(std::string sceneDirectory, nlohmann::json sceneConfig)
{
    ...
    this->lights = LoadLightData(sceneConfig);
    // Build the BVH
    this->buildBVH();
}
...
void Scene::intersectBVH(uint32_t nodeIdx, Ray &ray, Interaction& si,int option)
{
    BVHNode& node = this->nodes[nodeIdx];
    ...
    if (node.primCount != 0) {
        ...
        for (uint32_t i = 0; i < node.primCount; i++) {
            Interaction siIntermediate = this->surfaces[this->getIdx(i + node.firstPrim)].rayIntersect(ray,option);
